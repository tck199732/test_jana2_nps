...
async_filebuf::async_filebuf(int segsize, int segcount, int lookback) :
	...
	std::cout << THIS_ASYNCFB << "async_filebuf::async_filebuf(" << segsize << "," << segcount << "," << lookback << ")"
			  << std::endl;
#endif
...
std::streampos async_filebuf::seekoff(std::streamoff off, std::ios::seekdir way, std::ios::openmode which) {
#if VERBOSE_ASYNC_FILEBUF
