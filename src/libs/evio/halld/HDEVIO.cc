...
HDEVIO::HDEVIO(string fname, bool read_map_file, int verbose) : filename(fname), VERBOSE(verbose) {
	...
	if (read_map_file)
		ReadFileMap(); // check if a map file exists and read it if it does
...
HDEVIO::~HDEVIO() {
	...
	//    delete ifs.std::ios::rdbuf();  /// FIXME: comment out because jana4ml4fpga would not advance after reading one
	//    file
...
void HDEVIO::buff_read(char *s, streamsize nwords) {
	...
	if (_gcount > 0)
		memcpy((char *)s, (char *)fnext, _gcount);
	...
	if ((int64_t)Ncopied < nwords) {
		...
		if (mygcount > 0)
			memcpy((char *)s, (char *)fnext, mygcount);
		fnext += myNcopied;
...
bool HDEVIO::ReadBlock(void) {
	...
	if (buff_size < block_length) {
		if (block_length > buff_limit) {
			ClearErrorMessage();
			err_mess << "ERROR: EVIO block length greater than allocation limit (" << block_length << " > "
					 << block_length << " words)" << endl;
...
bool HDEVIO::read(uint32_t *user_buff, uint32_t user_buff_len, bool allow_swap) {
	...
	if (swap_needed)
		swap_block(&event_len, 1, &event_len);
...
bool HDEVIO::readSparse(uint32_t *user_buff, uint32_t user_buff_len, bool allow_swap) {
	...
	for (; sparse_block_iter != evio_blocks.end(); sparse_block_iter++, sparse_event_idx = 0) {
		...
		for (; sparse_event_idx < br.evio_events.size(); sparse_event_idx++) {
			...
			if (etype & event_type_mask)
				break;
		}
		if (sparse_event_idx >= br.evio_events.size())
			continue;
		...
		if ((!br.swap_needed) || (br.swap_needed && allow_swap)) {
			if ((user_buff[0] + 1) != event_len) {
				ClearErrorMessage();
				err_mess << "WARNING: EVIO bank indicates a different size than block header (" << event_len
						 << " != " << (user_buff[0] + 1) << ")";
				err_code = HDEVIO_EVENT_BIGGER_THAN_BLOCK;
				Nerrors++;
				Nbad_blocks++;
...
bool HDEVIO::readNoFileBuff(uint32_t *user_buff, uint32_t user_buff_len, bool allow_swap) {
	...
	if (br.evio_events.empty()) {
		...
		if (swap_needed)
			swap_block((uint32_t *)&bh, sizeof(bh) >> 2, (uint32_t *)&bh);
...
// GetNWordsLeftInFile
//------------------------
uint64_t HDEVIO::GetNWordsLeftInFile(void) { return (total_size_bytes - NB_next_pos) / 4; }
...
uint32_t HDEVIO::SetEventMask(string types_str) {
	...
	event_type_mask = 0;
	if (types_str.find("BOR") != string::npos)
		event_type_mask |= (1 << kBT_BOR);
	if (types_str.find("EPICS") != string::npos)
		event_type_mask |= (1 << kBT_EPICS);
	if (types_str.find("PHYSICS") != string::npos)
		event_type_mask |= (1 << kBT_PHYSICS);
...
// AddToEventMask
//------------------------
uint32_t HDEVIO::AddToEventMask(string type_str) { return 0; }
...
void HDEVIO::MapBlocks(bool print_ticker) {
	...
	while (ifs.good()) {
		...
		switch (tag) {
		case 0xFFD0:
			br.block_type = kBT_SYNC;
			break;
		case 0xFFD1:
			br.block_type = kBT_PRESTART;
			break;
		case 0xFFD2:
			br.block_type = kBT_GO;
			break;
		case 0xFFD3:
			br.block_type = kBT_PAUSE;
			break;
		case 0xFFD4:
			br.block_type = kBT_END;
			break;
		case 0x0060:
			br.block_type = kBT_EPICS;
			break;
		case 0x0070:
			br.block_type = kBT_BOR;
			break;
		case 0xFF50:
		case 0xFF51:
		case 0xFF70:
		...
		if (print_ticker) {
			if ((Nblocks % 500) == 0) {
				...
				if (Nblocks == 0)
					cout << endl;
				cout << Nblocks << " blocks scanned (" << read_MB << "/" << total_MB << " MB "
					 << (100 * read_MB / total_MB) << "%)     \r";
				cout.flush();
			}
		}
...
void HDEVIO::MapEvents(BLOCKHEADER_t &bh, EVIOBlockRecord &br) {
	...
	streampos pos = start_pos - (streampos)sizeof(BLOCKHEADER_t) +
					(streampos)(8 << 2); // (8<<2) is 8 word EVIO block header times 4bytes/word
	...
	for (uint32_t i = 0; i < bh.eventcnt; i++) {
		...
		if (i != 0) {
			...
			if (!ifs.good())
				break;
			if (br.swap_needed)
				swap_block((uint32_t *)eh, sizeof(EVENTHEADER_t) >> 2, (uint32_t *)eh);
		} else {
		...
		if (eh->event_len < 2) {
			...
			std::cout << "HDEVIO::MapEvents warning - " << "Attempt to swap bank with len<2 (len=" << eh->event_len
					  << " header=" << hex << eh->header << dec << " pos=" << pos << " tellg=" << ifs.tellg()
					  << " i=" << i << ")" << std::endl;
		...
		switch (tag) {
		case 0xFFD0:
			er.event_type = kBT_SYNC;
			break;
		case 0xFFD1:
			er.event_type = kBT_PRESTART;
			break;
		case 0xFFD2:
			er.event_type = kBT_GO;
			break;
		case 0xFFD3:
			er.event_type = kBT_PAUSE;
			break;
		case 0xFFD4:
			er.event_type = kBT_END;
			break;
		case 0x0060:
			er.event_type = kBT_EPICS;
			break;
		case 0x0070:
			er.event_type = kBT_BOR;
			break;
		case 0xFF50:
		case 0xFF51:
		case 0xFF70:
			...
			if (er.first_event < br.first_event)
				br.first_event = er.first_event;
			if (er.last_event > br.last_event)
				br.last_event = er.last_event;
			break;
		case 0xFF32:
			er.event_type = kBT_BOR;
			break;	 // CDAQ
...
void HDEVIO::PrintEVIOBlockHeader(void) {
	...
	cout << "------------------------" << endl;
	cout << " Block Length: " << HexStr(buff[0]) << " (" << buff[0] << " words = " << (buff[0] >> (10 - 2)) << " kB)"
		 << endl;
...
void HDEVIO::PrintStats(void) {
	...
	if (is_mapped) {
		...
		Nevents = 0;
		for (auto b : evio_blocks)
			Nevents += b.evio_events.size();
	}
...
void HDEVIO::PrintFileSummary(void) {
	...
	for (uint32_t i = 0; i < evio_blocks.size(); i++) {
		...
		for (uint32_t j = 0; j < br.evio_events.size(); j++) {
			...
			case kBT_PHYSICS:
				...
				if (er.first_event < first_event || first_event == 0)
					first_event = er.first_event;
				if (er.last_event > last_event)
					last_event = er.last_event;
				break;
			default:
				break;
			}

			//_DBG_ << "Block " << i << "  event " << j << "  " << er.last_event <<" - " << er.first_event << " = " <<
			// block_level << endl;
		}

		if ((Nunknown - Nunknown_prev) > 0)
			Nblockunknown++;
	}
	...
	for (; it != block_levels.end(); it++)
		ss << *it << ",";
	string sblock_levels = ss.str();
	if (!sblock_levels.empty())
		sblock_levels.erase(sblock_levels.length() - 1);
...
void HDEVIO::SaveFileMap(string fname) {
	...
	for (auto br : evio_blocks) {
		char line[512];
		sprintf(
			line, "0x%08x 0x%06x  %8" PRIu64 "  %8" PRIu64 "     %d", (uint32_t)br.pos, br.block_len, br.first_event,
			br.last_event, br.block_type
		);
		ofs << line << endl;

		for (auto er : br.evio_events) {
			sprintf(
				line, "+ 0x%08x 0x%06x 0x%08x %8" PRIu64 " %8" PRIu64 " %d", (uint32_t)er.pos, er.event_len,
				er.event_header, er.first_event, er.last_event, er.event_type
			);
			ofs << line << endl;
		}
	}
...
void HDEVIO::ReadFileMap(string fname, bool warn_if_not_found) {
	// Open input file
	if (VERBOSE > 4)
		cout << " Attempting to read EVIO map file \"" << fname << "\" for \"" << filename << "\"" << endl;
	if (fname == "") {
		...
		for (string f : fnames) {
			if (VERBOSE > 2)
				cout << "Checking for EVIO map file: " << f << " ...";
	...
	if (!mifs.is_open()) {
		if (warn_if_not_found)
			cerr << "Unable to open \"" << fname << "\" for reading!" << endl;
		return;
	}
	...
	while (getline(mifs, line)) {
		...
		if (line.find("#") == 0)
			continue;
		if (line.find("Start of block data") != string::npos)
			break;
	}
	...
	while (getline(mifs, s)) {
		stringstream ss(s);
		if (ss.str().find(eof_string) != string::npos)
			break; // end of map trailer
		if (ss.str().find("#") == 0)
		...
		if (ss.str().find("+") == 0) {
			...
			ss << hex;
			ss >> tmp64;
			er.pos = tmp64; // operator>> won't stream directly to streampos
			ss >> er.event_len;
			ss >> er.event_header;
			ss << dec;
			ss >> er.first_event;
			ss >> er.last_event;
			ss >> tmp64;
			er.event_type = (BLOCKTYPE)tmp64; // operator>> won't stream directly to BLOCKTYPE
			br.evio_events.push_back(er);
		} else {
			// EVIO Block Record
			...
			ss << hex;
			ss >> tmp64;
			br.pos = tmp64; // operator>> won't stream directly to streampos
			ss >> br.block_len;
			ss << dec;
			ss >> br.first_event;
			ss >> br.last_event;
			ss >> tmp64;
			br.block_type = (BLOCKTYPE)tmp64; // operator>> won't stream directly to BLOCKTYPE
		}
	}
	if (!br.evio_events.empty())
		evio_blocks.push_back(br);
